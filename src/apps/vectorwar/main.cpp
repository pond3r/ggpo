...
{
    MessageBox(NULL,
        L"Syntax: vectorwar.exe <local port> <num players> ('local' | <steam id>)*\n",
    ...
    _In_ int)
{
    ...
    if (wcscmp(__wargv[offset], L"spectate") == 0) {
        // TODO: Spectors not yet supported for Steam integration
        Syntax();
        return 1;
        ...
        for (i = 0; i < num_players; i++) {
            ...
            uint64 id = _wtoi64(arg);
            if (id == 0) {
                Syntax();
                return 1;
            }
            players[i].u.remote.steam_id = id;
        }
