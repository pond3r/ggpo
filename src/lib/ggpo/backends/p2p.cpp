   ...
   if (!_sync.InRollback()) {
      ...
      if (!_synchronizing) {
         ...
         if (current_frame > _next_recommended_sleep) {
            int interval = 0;
            for (int i = 0; i < _num_players; i++) {
               interval = std::max(interval, _endpoints[i].RecommendFrameDelay());
            }
...
int Peer2PeerBackend::Poll2Players(int current_frame)
{
   ...
   for (i = 0; i < _num_players; i++) {
      ...
      if (!_local_connect_status[i].disconnected) {
         total_min_confirmed = std::min(_local_connect_status[i].last_frame, total_min_confirmed);
      }
   ...
   for (queue = 0; queue < _num_players; queue++) {
      ...
      for (i = 0; i < _num_players; i++) {
         ...
         if (_endpoints[i].IsRunning()) {
            ...
            queue_min_confirmed = std::min(last_received, queue_min_confirmed);
      ...
      if (!_local_connect_status[queue].disconnected) {
         queue_min_confirmed = std::min(_local_connect_status[queue].last_frame, queue_min_confirmed);
      }
      ...
      if (queue_connected) {
         total_min_confirmed = std::min(queue_min_confirmed, total_min_confirmed);
      } else {
