...
UdpProtocol::UdpProtocol() : _local_frame_advantage(0),
                             ...
                             _udp(NULL)
{
   ...
   char *delay;
   size_t requiredSize;
   getenv_s(&requiredSize, NULL, 0, "ggpo.network.delay");
   if (requiredSize == 0)
   {
      printf("ggpo.network.delay environment variable undefined!\n");
      // exit(1);
   }
   delay = (char *)malloc(requiredSize * sizeof(char));
   if (!delay)
   {
      printf("Failed to allocate memory! ggpo.network.delay\n");
      exit(1);
   }
   getenv_s(&requiredSize, delay, requiredSize, "ggpo.network.delay");

   _send_latency = *delay ? std::atoi(delay) : 0;
   free(delay);

   char *oop_pct;
   getenv_s(&requiredSize, NULL, 0, "ggpo.oop.percent");
   if (requiredSize == 0)
   {
      printf("ggpo.oop.percent environment variable undefined!\n");
      // exit(1);
   }
   oop_pct = (char *)malloc(requiredSize * sizeof(char));
   if (!oop_pct)
   {
      printf("Failed to allocate memory! ggpo.oop.percent\n");
      exit(1);
   }
   getenv_s(&requiredSize, oop_pct, requiredSize, "ggpo.oop.percent");
   _oop_percent = *oop_pct ? std::atoi(oop_pct) : 0;
   free(oop_pct);
}
                       ...
                       UdpMsg::connect_status *status)
{
   ...
   _peer_addr.sin_addr.s_addr = inet_pton(AF_INET, ip, ipbuf);
...
bool UdpProtocol::HandlesMsg(sockaddr_in &from,
                             UdpMsg *msg)
{
   if (!_udp)
   {
